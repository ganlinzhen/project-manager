  ...
  return (
    <main className="page detail-page">
      ...
      <header className="detail-heading">
        <div><div className="detail-heading__meta"><span className={`status status--${task.status}`}>{labels[task.status]}</span><span>{task.id}</span><span>{detail.project.name}</span></div><h1>{task.title}</h1></div>
