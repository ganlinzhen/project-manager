...
var require_Collection = __commonJS({
  "../../node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/nodes/Collection.js"(exports) {
    ...
    var identity = require_identity();
    var Node = require_Node();
    function collectionFromPath(schema, path9, value2) {
      let v = value2;
    ...
    var Document = class _Document {
      ...
      setIn(path9, value2) {
        if (Collection.isEmptyPath(path9)) {
          ...
          this.contents = Collection.collectionFromPath(this.schema, Array.from(path9), value2);
        } else if (assertCollection(this.contents)) {
...
// ../core/src/errors.ts
var WorkManagerError = class extends Error {
  code;
  recoverable;
  suggestedCommand;
  details;
...
function toWorkManagerError(error, fallbackCode = "UNEXPECTED_ERROR") {
...
// ../core/src/paths.ts
function assertWithinRoots(candidate, roots) {
...
function defaultDatabasePath() {
  const appData = process.env.WM_APP_DATA_DIR ?? path.join(process.env.HOME ?? process.cwd(), "Library", "Application Support", "work-manager");
  return path.join(appData, "work-manager.db");
}
async function assertRealPathWithinRoots(candidate, roots) {
...
var ArtifactService = class {
  constructor(managerRoot, repository) {
    this.managerRoot = managerRoot;
    this.repository = repository;
    this.artifactsRoot = path2.join(managerRoot, "data", "artifacts");
  }
  artifactsRoot;
  async createBaseArtifacts(task) {
    const directory = assertWithinRoots(path2.join(this.artifactsRoot, task.id), [this.artifactsRoot]);
...
`,
    ...
    for (const kind of Object.keys(fileNames)) {
      const artifactPath = path2.join(directory, fileNames[kind]);
      try {
        await readFile(artifactPath);
      } catch {
        await this.atomicWrite(artifactPath, templates[kind]);
      }
  ...
  async updateProgress(taskId, current, next) {
    const artifactPath = assertWithinRoots(path2.join(this.artifactsRoot, taskId, fileNames.progress), [this.artifactsRoot]);
    await assertRealPathWithinRoots(path2.dirname(artifactPath), [this.artifactsRoot]);
...
// ../core/src/command-runner.ts
import { spawn } from "node:child_process";
var SystemCommandRunner = class {
  run(argv, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(new WorkManagerError("COMMAND_TIMEOUT", `${argv[0]} \u6267\u884C\u8D85\u65F6`, {
          recoverable: true,
          details: { command: argv[0], timeoutMs: options.timeoutMs ?? 3e4 }
        }));
      ...
      child.on("close", (code) => {
        ...
        if (result.code === 0) resolve(result);
          ...
          recoverable: true,
          details: { command: argv[0], exitCode: result.code }
        }));
      });
    });
  }
...
var ParseInputLazyPath = class {
  constructor(parent, value2, path9, key) {
    this._cachedPath = [];
    this.parent = parent;
    this.data = value2;
    this._path = path9;
...
var serviceSchema = external_exports.object({
  cwd: external_exports.string().min(1).default("."),
  startCommand: external_exports.union([external_exports.string().min(1), external_exports.array(external_exports.string().min(1)).min(1)]),
  healthCheckUrl: external_exports.string().url().optional(),
  port: external_exports.number().int().positive().max(65535).optional()
});
var projectSchema = external_exports.object({
  ...
  issue: external_exports.object({
    provider: external_exports.enum(["github", "gitlab", "none"]),
    repository: external_exports.string().min(1).optional(),
    labels: external_exports.object({
      feature: external_exports.array(external_exports.string()).optional(),
      bug: external_exports.array(external_exports.string()).optional(),
      chore: external_exports.array(external_exports.string()).optional()
    }).optional()
  }),
  development: external_exports.object({ services: external_exports.record(external_exports.string().regex(/^[a-z][a-z0-9-]*$/), serviceSchema).default({}) }).default({ services: {} })
}).superRefine((value2, context) => {
  if (value2.issue.provider !== "none" && !value2.issue.repository) {
    context.addIssue({ code: external_exports.ZodIssueCode.custom, path: ["issue", "repository"], message: "\u542F\u7528 Issue \u63D0\u4F9B\u65B9\u65F6\u5FC5\u987B\u8BBE\u7F6E repository" });
  }
  if (!path3.isAbsolute(value2.repositoryPath)) {
    context.addIssue({ code: external_exports.ZodIssueCode.custom, path: ["repositoryPath"], message: "repositoryPath \u5FC5\u987B\u662F\u7EDD\u5BF9\u8DEF\u5F84" });
  }
  if (value2.worktreeRoot && !path3.isAbsolute(value2.worktreeRoot)) {
    context.addIssue({ code: external_exports.ZodIssueCode.custom, path: ["worktreeRoot"], message: "worktreeRoot \u5FC5\u987B\u662F\u7EDD\u5BF9\u8DEF\u5F84" });
  }
  for (const [serviceKey, service] of Object.entries(value2.development.services)) {
    try {
      parseCommandLine(service.startCommand);
    } catch (error) {
      context.addIssue({ code: external_exports.ZodIssueCode.custom, path: ["development", "services", serviceKey, "startCommand"], message: error instanceof Error ? error.message : String(error) });
    }
  }
});
async function loadProjectConfig(filePath) {
  try {
    const raw = import_yaml.default.parse(await readFile2(filePath, "utf8"));
...
async function loadProjectConfigs(directory) {
  ...
  for (const file of files.filter((name) => /\.ya?ml$/i.test(name)).sort()) {
    const config = await loadProjectConfig(path3.join(directory, file));
...
var WorkManagerDatabase = class {
  connection;
  constructor(filePath) {
    mkdirSync(path4.dirname(filePath), { recursive: true });
  ...
  migrate() {
    ...
    `);
    const serviceColumns = this.connection.prepare("PRAGMA table_info(development_services)").all();
...
var DoctorService = class {
  constructor(repository, projectResolver, processes, external) {
    this.repository = repository;
  ...
  async check(taskId) {
    ...
    if (progressArtifact && await exists(progressArtifact.path)) {
      const progress = await readFile3(progressArtifact.path, "utf8");
    ...
    if (task.issueNumber && task.issueUrl && this.external?.issues) {
      try {
        await this.external.issues.getForTask(taskId);
        issueMessage = "Issue \u53EF\u8BBF\u95EE";
      } catch (error) {
        issueOk = false;
        issueMessage = error instanceof Error ? error.message : String(error);
      }
    }
    const deadServices = this.repository.listServices(taskId).filter(
      (service) => ["running", "unhealthy", "starting"].includes(service.status) && (!service.pid || !service.processIdentity || !this.processes.isRunning(service.pid, service.processIdentity))
    );
    const checks = [
      { key: "database", ok: Boolean(project), message: project ? "\u4EFB\u52A1\u548C\u9879\u76EE\u8BB0\u5F55\u53EF\u8BFB\u53D6" : "\u9879\u76EE\u8BB0\u5F55\u7F3A\u5931", suggestedCommand: project ? void 0 : `wm project validate ${task.projectId} --json` },
      { key: "artifacts", ok: artifacts.length === 5 && missingArtifacts.length === 0 && artifactContentOk, message: missingArtifacts.length ? `\u7F3A\u5C11 ${missingArtifacts.length} \u4E2A\u4EA7\u7269\u6587\u4EF6` : !artifactContentOk ? "progress.md \u4E0E SQLite \u8FDB\u5C55\u5B57\u6BB5\u4E0D\u4E00\u81F4" : `${artifacts.length}/5 \u4E2A\u4EA7\u7269\u53EF\u7528`, suggestedCommand: missingArtifacts.length || artifacts.length !== 5 || !artifactContentOk ? `wm task retry ${taskId} --json` : void 0 },
      { key: "worktree", ok: worktreeOk, message: worktreeMessage, suggestedCommand: worktreeOk ? void 0 : `wm task retry ${taskId} --json` },
      { key: "issue", ok: issueOk, message: issueMessage, suggestedCommand: issueOk ? void 0 : `wm task retry ${taskId} --json` },
      { key: "services", ok: deadServices.length === 0, message: deadServices.length ? `${deadServices.length} \u4E2A\u670D\u52A1 PID \u4E0D\u5B58\u5728` : "\u670D\u52A1\u8BB0\u5F55\u4E00\u81F4", suggestedCommand: deadServices.length ? `wm env status ${taskId} --json` : void 0 }
    ];
...
// ../core/src/environment-service.ts
import { spawn as spawn2 } from "node:child_process";
import { execFileSync } from "node:child_process";
import path5 from "node:path";
var SystemProcessController = class {
  async start(argv, cwd) {
    return new Promise((resolve, reject) => {
      const child = spawn2(argv[0], argv.slice(1), { cwd, detached: true, shell: false, stdio: "ignore" });
...
var EnvironmentService = class {
  constructor(repository, projectResolver, processes) {
    this.repository = repository;
    this.projectResolver = projectResolver;
    this.processes = processes;
  }
  async start(taskId, serviceKey) {
    const task = this.repository.requireTask(taskId);
    ...
    const cwd = await assertRealPathWithinRoots(path5.resolve(task.worktreePath, config.cwd), [task.worktreePath]);
    ...
    try {
      const starting = {
        ...
        port: config.port ?? this.portFromUrl(config.healthCheckUrl),
      ...
      const running = { ...starting, pid: startedPid, processIdentity: startedIdentity, status: "running" };
      ...
      if (startedPid && startedIdentity && this.processes.isRunning(startedPid, startedIdentity)) {
        try {
          await this.processes.stop(startedPid, startedIdentity);
        } catch {
        }
      }
      const failed = {
  ...
  async stop(taskId, serviceKey) {
    ...
    try {
      ...
      const stopped = { ...existing, pid: null, processIdentity: null, status: "stopped", stoppedAt: now() };
...
var IssueService = class {
  constructor(repository, projectResolver, runner) {
    this.repository = repository;
    this.projectResolver = projectResolver;
    this.runner = runner;
  }
...
function mapTask(row) {
  return {
    ...
    issueProvider: row.issue_provider,
...
function redact(value2) {
  ...
  if (value2 && typeof value2 === "object") {
    return Object.fromEntries(Object.entries(value2).filter(([key]) => !/token|cookie|authorization|header|environment/i.test(key)).map(([key, item]) => [key, redact(item)]));
  }
  return value2;
}
var TaskRepository = class {
  ...
  registerProject(project) {
    ...
    `).run(project.id, JSON.stringify(project), now2());
  }
  getProject(projectId) {
    const row = this.database.connection.prepare("SELECT config_json FROM projects WHERE id = ?").get(projectId);
    return row ? JSON.parse(String(row.config_json)) : null;
  }
  listProjects() {
    return this.database.connection.prepare("SELECT config_json FROM projects ORDER BY id").all().map((row) => JSON.parse(String(row.config_json)));
  }
  createTask(project, input) {
    return this.database.transaction(() => {
      const update = this.database.connection.prepare("UPDATE projects SET number_counter = number_counter + 1, updated_at = ? WHERE id = ?").run(now2(), project.id);
      if (update.changes !== 1) throw new WorkManagerError("PROJECT_NOT_REGISTERED", `\u9879\u76EE\u672A\u6CE8\u518C\uFF1A${project.id}`);
      const counter = this.database.connection.prepare("SELECT number_counter FROM projects WHERE id = ?").get(project.id);
      ...
      `).run(
        ...
        input.type,
        input.priority,
        input.requirementSummary ?? null,
        project.issue.provider,
        input.createIssue ? 1 : 0,
        input.createWorktree ? 1 : 0,
        timestamp,
        timestamp
      );
  ...
  listTasks(filters = {}) {
    ...
    if (filters.query) {
      where.push("(title LIKE ? OR id LIKE ? OR next_action LIKE ?)");
  ...
  updateTask(id, values) {
    ...
    const params = entries.map(([, value2]) => value2 ?? null);
    this.database.connection.prepare(`UPDATE tasks SET ${clauses.join(", ")}, updated_at = ? WHERE id = ?`).run(...params, now2(), id);
    return this.requireTask(id);
  }
  transition(id, to, message) {
    return this.database.transaction(() => {
  ...
  appendEvent(taskId, type, success, message = null, metadata = {}) {
    this.database.connection.prepare("INSERT INTO task_events(task_id, type, success, message, metadata_json, created_at) VALUES (?, ?, ?, ?, ?, ?)").run(taskId, type, success ? 1 : 0, message, JSON.stringify(redact(metadata)), now2());
  }
  listEvents(taskId) {
    return this.database.connection.prepare("SELECT * FROM task_events WHERE task_id = ? ORDER BY id").all(taskId).map((row) => ({
      id: Number(row.id),
  ...
  upsertArtifact(taskId, kind, artifactPath) {
    this.database.connection.prepare(`
    ...
    `).run(taskId, kind, artifactPath, now2());
  }
  listArtifacts(taskId) {
    return this.database.connection.prepare("SELECT * FROM task_artifacts WHERE task_id = ? ORDER BY kind").all(taskId).map((row) => ({
      taskId: String(row.task_id),
      kind: row.kind,
  ...
  getService(taskId, serviceKey) {
    const row = this.database.connection.prepare("SELECT * FROM development_services WHERE task_id=? AND service_key=?").get(taskId, serviceKey);
    return row ? this.mapService(row) : null;
  }
  listServices(taskId) {
    return this.database.connection.prepare("SELECT * FROM development_services WHERE task_id=? ORDER BY service_key").all(taskId).map((row) => this.mapService(row));
  }
  mapService(row) {
    return {
      taskId: String(row.task_id),
      ...
      processIdentity: row.process_identity == null ? null : String(row.process_identity),
      port: row.port == null ? null : Number(row.port),
      healthCheckUrl: row.health_check_url == null ? null : String(row.health_check_url),
      status: row.status,
...
// ../core/src/task-service.ts
import path6 from "node:path";
var TaskService = class {
  constructor(repository, artifacts, projectResolver, external) {
    this.repository = repository;
  ...
  async createTask(input) {
    ...
    try {
      ...
      if (input.createIssue) {
        await this.external?.issues?.createForTask(task.id);
        completedSteps.push("issue_created");
      }
      if (input.createWorktree) {
        await this.external?.workspace?.createWorktree(task.id);
        completedSteps.push("worktree_created");
      }
  ...
  async updateProgress(taskId, values) {
  ...
  resumeTask(taskId) {
    return this.changeStatus(taskId, "in_progress", "\u4EFB\u52A1\u5DF2\u6062\u590D");
  }
  completeTask(taskId) {
    return this.changeStatus(taskId, "done", "\u4EFB\u52A1\u5DF2\u5B8C\u6210\uFF0C\u8D44\u6E90\u4FDD\u7559");
  }
  reopenTask(taskId) {
    return this.changeStatus(taskId, "in_progress", "\u4EFB\u52A1\u5DF2\u91CD\u5F00");
  }
  listServices(taskId) {
    const task = this.repository.requireTask(taskId);
    const project = this.projectResolver(task.projectId) ?? this.repository.getProject(task.projectId);
    ...
    const configured = Object.entries(project.development.services).map(([serviceKey, config]) => stored.get(serviceKey) ?? {
      ...
      cwd: path6.resolve(task.worktreePath ?? project.repositoryPath, config.cwd),
      pid: null,
      processIdentity: null,
...
import { mkdir as mkdir2, realpath as realpath2 } from "node:fs/promises";
import path7 from "node:path";
function slug(value2) {
  const normalized = value2.normalize("NFKD").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 40);
...
var WorkspaceService = class {
  ...
  async createWorktree(taskId) {
    ...
    try {
      ...
      const worktreesRoot = project.worktreeRoot ?? path7.join(path7.dirname(project.repositoryPath), ".work-manager-worktrees", project.id);
      const branchName = `wm/${task.id.toLowerCase()}-${slug(task.title)}`;
      await mkdir2(worktreesRoot, { recursive: true });
      const canonicalRoot = await realpath2(worktreesRoot);
      const worktreePath = assertWithinRoots(path7.join(canonicalRoot, task.id), [canonicalRoot]);
...
function value(args, name, required = false) {
  ...
  const found = index >= 0 ? args[index + 1] : void 0;
...
async function findManagerRoot(start) {
  let current = path8.resolve(start);
  while (true) {
    try {
      await access2(path8.join(current, "pnpm-workspace.yaml"));
      await access2(path8.join(current, "projects"));
      return current;
    } catch {
      const parent = path8.dirname(current);
      if (parent === current) return path8.resolve(start);
      current = parent;
    }
  }
}
async function createRuntime(options) {
  ...
  const managerRoot = configuredRoot ? path8.resolve(configuredRoot) : await findManagerRoot(process.cwd());
  const projectsDir = path8.resolve(options.projectsDir ?? process.env.WM_PROJECTS_DIR ?? path8.join(managerRoot, "projects"));
  const databasePath = path8.resolve(options.databasePath ?? process.env.WM_DATABASE_PATH ?? defaultDatabasePath());
  ...
  const resolveProject = (projectId) => projects.get(projectId) ?? repository.getProject(projectId) ?? void 0;
...
async function dispatch(args, runtime) {
  ...
  if (scope === "project" && action === "validate") {
    ...
    for (const [serviceKey, service] of Object.entries(project.development.services)) {
      const cwd = await assertRealPathWithinRoots(path8.resolve(project.repositoryPath, service.cwd), [project.repositoryPath]);
      const argv = parseCommandLine(service.startCommand);
      if (path8.isAbsolute(argv[0])) await access2(argv[0]);
  ...
  if (scope === "task") {
    if (action === "create") {
      ...
      const type = value(args, "--type") ?? "feature";
      ...
      return runtime.tasks.createTask({
        ...
        type,
        priority,
        requirementSummary: value(args, "--requirement"),
        createIssue: flag(args, "--create-issue"),
        createWorktree: flag(args, "--create-worktree")
      });
    }
    if (action === "list") {
...
async function executeCli(args, options = {}, emit = () => {
}) {
  let runtime;
  try {
    runtime = await createRuntime(options);
    const response = { ok: true, data: await dispatch(args, runtime) };
    emit(response);
    return response;
  } catch (error) {
...
\u5EFA\u8BAE\uFF1A${response.error.suggestedCommand}` : ""}`;
  ...
  const task = data && "task" in data ? data.task : void 0;
